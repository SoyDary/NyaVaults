...
public class Comandos implements CommandExecutor {
				...
				if(a.length == 0) {
					...
					if(player.hasPermission("nv.admin")) {
						...
						if(plugin.getDataManager().Players.getString("Players."+a[0]) != null) { 
							try {
								plugin.getGUI().vault(player, a[0]);
							} catch (IOException e) {
								e.printStackTrace();
							}						
						} else {
					...
					String uuid = null;
					String uuid2 = null;
						for(String key : plugin.getDataManager().Players.getConfigurationSection("Players").getKeys(false)){								
							if(plugin.getDataManager().Players.getString("Players."+key+".name").equalsIgnoreCase(a[0])) {
								if(uuid == null) uuid = key; else uuid2 = key;	
							}
						}
				    if(uuid2 == null) {
					if(uuid != null)  {
						try {
							plugin.getGUI().vault(player, uuid);
						} catch (IOException e) {
							e.printStackTrace();
						}				
					...
					player.sendMessage("§cEste no es un jugador registrado");
						...
						if(plugin.getUtils().isPremium(uuid)) uuid = uuid+" §e(Premium)";
						if(plugin.getUtils().isPremium(uuid2)) uuid2 = uuid2+" §e(Premium)";
						String realName = plugin.getDataManager().Players.getString("Players."+uuid+".name");
						player.sendMessage(plugin.getUtils().color(" &#33ccff&l&oHay 2 cuentas registradas con este nombre:"));
						player.sendMessage("");
			...
			if(l.equalsIgnoreCase("mycookies") || l.equalsIgnoreCase("galletas")) {
				...
				if(a.length == 0) {
					...
					player.sendMessage("§cEste no es un jugador registrado");
						...
						HoverEvent ev1 = new HoverEvent(HoverEvent.Action.SHOW_TEXT, cba1.create());
					    t1.setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/galletas "+uuid.split(" ")[0]));    
					    t1.setHoverEvent(ev1);
						TextComponent t2 = new TextComponent("§9↳ §b"+realName+"§8: §7"+uuid2);
					    ComponentBuilder cba2 = new ComponentBuilder();
					    cba2.append("§7§o/galletas "+uuid2.split(" ")[0]);
						HoverEvent ev2 = new HoverEvent(HoverEvent.Action.SHOW_TEXT, cba2.create());
					    t2.setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/galletas "+uuid2.split(" ")[0]));    
					    t2.setHoverEvent(ev2);					    
					    end.addExtra((BaseComponent)t1);
					    end.addExtra("\n");
					    end.addExtra((BaseComponent)t2);
			...
			if(l.equalsIgnoreCase("marrychest") || l.equalsIgnoreCase("mc")) {
				...
				if(a.length == 0) {
					...
					player.sendMessage("§cEste no es un jugador registrado");
						...
						HoverEvent ev1 = new HoverEvent(HoverEvent.Action.SHOW_TEXT, cba1.create());
					    t1.setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/marrychest "+uuid.split(" ")[0]));    
					    t1.setHoverEvent(ev1);
						TextComponent t2 = new TextComponent("§9↳ §b"+realName+"§8: §7"+uuid2);
					    ComponentBuilder cba2 = new ComponentBuilder();
					    cba2.append("§7§o/marrychest "+uuid2.split(" ")[0]);
						HoverEvent ev2 = new HoverEvent(HoverEvent.Action.SHOW_TEXT, cba2.create());
					    t2.setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/marrychest "+uuid2.split(" ")[0]));    
					    t2.setHoverEvent(ev2);					    
					    end.addExtra((BaseComponent)t1);
					    end.addExtra("\n");
					    end.addExtra((BaseComponent)t2);
			...
			if(l.equalsIgnoreCase("mymaps") || l.equalsIgnoreCase("mapas")) {
				if(a.length == 0) {
					...
					player.sendMessage("§cEste no es un jugador registrado");
						...
						HoverEvent ev1 = new HoverEvent(HoverEvent.Action.SHOW_TEXT, cba1.create());
					    t1.setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/mapas "+uuid.split(" ")[0]));    
					    t1.setHoverEvent(ev1);
						TextComponent t2 = new TextComponent("§9↳ §b"+realName+"§8: §7"+uuid2);
					    ComponentBuilder cba2 = new ComponentBuilder();
					    cba2.append("§7§o/mapas "+uuid2.split(" ")[0]);
						HoverEvent ev2 = new HoverEvent(HoverEvent.Action.SHOW_TEXT, cba2.create());
					    t2.setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/mapas "+uuid2.split(" ")[0]));    
					    t2.setHoverEvent(ev2);					    
					    end.addExtra((BaseComponent)t1);
					    end.addExtra("\n");
					    end.addExtra((BaseComponent)t2);
	...
	public void seassonItem(Player p) {
		...
		String team_line = PlaceholderAPI.setPlaceholders(p, "%equipo_signcolor%» %equipo_color%Equipo: %equipo_prefix%%equipo_displayname%&r %equipo_signcolor%«");
		...
		String nyafecha = "&#ffff99"+fecha[0]+"&#ffff1a/&#ffff99"+fecha[1]+"&#ffff1a/&#ffff99"+fecha[2]+" &#b3ffec| &#ffd966"+hora[0]+"&6:&#ffd966"+hora[1]+"&6:&#ffd966"+hora[2];
		...
		if(team_name != "") lore.add(plugin.getUtils().clr(team_line));
		lore.add(plugin.getUtils().clr("&#FFBF80~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~"));
		...
		item.setItemMeta(im);
		sendItem(p, item);
		p.sendMessage(plugin.getUtils().clr("&#ffff00&lK&#ffff33&lo&#ffff4d&lk&#ffff66&lo&#ffff80&lr&#ffff99&li&#ff8000&lC&#ff9933&lr&#ffb366&la&#ffbf80&lf&#ffcc99&l&lt &b» &#ffff99Recibiste un item especial por el final de temporada!"));
		p.playSound(p.getLocation(), Sound.ENTITY_PLAYER_LEVELUP, 1, 10);
			
	}
	  public void sendItem(final Player p, final ItemStack item) {
		    Boolean isFull = Boolean.valueOf(!Arrays.<ItemStack>asList(p.getInventory().getStorageContents()).contains(null));
	  ...
	  public boolean checkspace(Player p, ItemStack item, int amount) {
		    ...
		    for (ItemStack i : p.getInventory()) {
		      if (i != null && !i.getType().equals(Material.AIR)) {
		        ...
		        ii.setAmount(1);
		        ii2.setAmount(1);
		        if ((ii.equals(ii2) || ii == ii2) && 
		          i.getAmount() < i.getMaxStackSize()) {
	...
	public void MarkMap(Player p) {	
		...
		if (item != null && item.getType().toString().equals("FILLED_MAP")) {			
		    ...
		    for (String k : tag.d()) {
		        ...
		        if (k.equalsIgnoreCase("SavedMap")) {
		        	saved = nbtbase.toString();
		        } 
		      } 
			File miau;
			miau = new File("KokoriCraft/data/map_"+id+".dat");
		    	...
		    	if(miau.exists()) {
				    if(plugin.getDataManager().Maps.get("SavedMaps.map_"+id) == null) {
				    	...
				    	try {
							...
							p.sendMessage(plugin.getUtils().color("&b▆ &#ff9933¡Hubo un problema intentando marcar este mapa!"));
							e.printStackTrace();
						}
	  ...
	  public void setTag(int newID, Player p) {
		  ...
		  net.minecraft.world.item.ItemStack nmsitem = ItemUtils.getNMSItem(SelectedItem);
		  NBTTagCompound tag = nmsitem.u();
		  if (tag == null)
		  ...
		  ItemMeta im = item2.getItemMeta();
			...
			if (im.hasLore()) {
				 ...
				 lore.add(plugin.getUtils().color("&#ffff00&lK&#ffff33&lo&#ffff4d&lk&#ffff66&lo&#ffff80&lr&#ffff99&li&#ff8000&lC&#ff9933&lr&#ffb366&la&#ffbf80&lf&#ffcc99&l&lt #ffcc99&lｓ&#ff8c1a⁶"));
				 im.setLore(lore);
